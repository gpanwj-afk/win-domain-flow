# win-domain-flow 0.6.1 Windows 安装与验证指南

## 0.6.1 稳定性说明

0.6.1 不扩张浏览器权限或采集范围，重点收紧数据一致性与兼容保护：

- 基础域名总量与“应用 → 域名”明细改为同一 SQLite 事务提交；任一部分失败时整批回滚，不再留下半边已入库的数据。
- 打开应用层数据库前先检查应用 schema 版本；未来版本或非法版本会在应用层建表/建索引前拒绝，防止旧程序改写新数据库。
- “今日 / 本月 / 最近 7 天 / 最近 30 天”统一按 UTC 自然日边界统计，与数据库实际的 UTC 日桶保持一致。
- CLI 与独立 Receiver 的相对 `--db` 路径会先转换为绝对路径，避免工作目录下相对数据库初始化失败。

浏览器扩展代码、权限和协议与 0.6 相同；已经加载 0.6 发行包扩展的用户无需因为本次桌面稳定性修复新增权限。
