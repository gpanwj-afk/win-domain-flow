...
0.6.1 不扩大浏览器权限或采集范围，重点修复长期运行和升级场景的数据一致性：

- 基础域名总量与“应用 → 域名”明细现在在同一个 SQLite 事务内提交；任一部分失败时整批回滚，不再出现只写入半边累计数据的情况。
- 应用层数据库在执行建表或建索引前先检查 schema 版本；未来版本或非法版本会直接拒绝，避免旧程序修改新版本数据库。
- “今日 / 本月 / 最近 7 天 / 最近 30 天”统一采用 UTC 自然日边界，与数据库实际按 UTC 日持久化的粒度一致，避免非 UTC 时区月初多计前一日。
- CLI 与独立 Receiver 接收相对 `--db` 路径时会先转换为绝对路径，避免工作目录下的相对数据库路径初始化失败。
- `Cargo.lock` 由 Cargo 自身重新生成并经过差异复核，0.6.1 仅改变本项目根包版本，不漂移第三方依赖。

浏览器扩展代码、权限和协议与 0.6 相同；本次桌面稳定性修复不要求新增扩展权限。
...
若从 0.5 或更早版本升级，建议在扩展管理页重新加载当前发行包中的扩展。0.6 起使用固定公钥，因此后续发行包的扩展身份稳定。
...
基础流量按 UTC 自然日持久化，因此“今日 / 本月 / 最近 7 天 / 最近 30 天”也按 UTC 日/月边界统计。这样统计查询与已落库的日桶完全一致，不会在本地时区月初错误纳入前一个 UTC 日。
...
也可以传相对路径，例如 `--db .\domainflow-e2e.db`；0.6.1 会在 Receiver 启动前将其解析为当前工作目录下的绝对路径。
...
target\release\win-domain-flow-browser-receiver.exe
```
