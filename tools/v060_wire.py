...
def replace_test_function(text: str, function_name: str, replacement: str) -> str:
    needle = f"    #[test]\n    fn {function_name}()"
    start = text.index(needle)
    next_test = text.find("    #[test]", start + len(needle))
    if next_test < 0:
        raise SystemExit(f"next test after {function_name} not found")
    return text[:start] + replacement.rstrip() + "\n\n" + text[next_test:]


path = Path("src/browser_activity.rs")
text = path.read_text()
text = replace_once(text, "use serde::Deserialize;", "use serde::{Deserialize, Serialize};", "serde import")
text = replace_once(
    text,
...
for old, new, label in [
    ("page_url = excluded.page_url", "page_url = COALESCE(excluded.page_url, browser_request_event.page_url)", "page URL merge"),
    ("initiator = excluded.initiator", "initiator = COALESCE(excluded.initiator, browser_request_event.initiator)", "initiator merge"),
    ("method = excluded.method", "method = COALESCE(excluded.method, browser_request_event.method)", "method merge"),
    ("resource_type = excluded.resource_type", "resource_type = COALESCE(excluded.resource_type, browser_request_event.resource_type)", "resource type merge"),
    ("status_code = excluded.status_code", "status_code = COALESCE(excluded.status_code, browser_request_event.status_code)", "status merge"),
    ("mime = excluded.mime", "mime = COALESCE(excluded.mime, browser_request_event.mime)", "mime merge"),
    ("error_text = excluded.error_text", "error_text = COALESCE(excluded.error_text, browser_request_event.error_text)", "error merge"),
]:
...
server_start = text.index("#[derive(Debug, Clone)]\npub struct BrowserServerStatus")
server_end = text.index("struct HttpRequest {", server_start)
server_section = r'''#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct BrowserServerStatus {
    pub product: String,
    pub pid: u32,
    pub port: u16,
...
'''
text = text[:server_start] + server_section + text[server_end:]

origin_start = text.index("fn allowed_extension_origin(")
origin_end = text.index("fn cors_headers(", origin_start)
origin_fn = r'''fn require_extension_origin(origin: Option<&str>) -> Result<String, BrowserActivityError> {
    let origin = origin
        .map(str::trim)
    ...
    if normalized == chrome || normalized == edge {
        ...
        Err(BrowserActivityError::Protocol("unexpected extension origin"))
    }
}

'''
text = text[:origin_start] + origin_fn + text[origin_end:]
text = replace_once(
    text,
    '        400 => "Bad Request",\n        404 => "Not Found",',
    '        400 => "Bad Request",\n        403 => "Forbidden",\n        404 => "Not Found",',
...
text = replace_test_function(text, "only_extension_origins_are_allowed", origin_test)
...
text = text[:insert_at] + monotonic_test + text[insert_at:]

receiver_tests = r'''    fn receiver_request(
        port: u16,
        method: &str,
        path: &str,
        origin: Option<&str>,
    ...
    fn receiver_rejects_missing_and_wrong_origins_but_accepts_packaged_extension() {
        ...
        assert_eq!(storage.recent_requests(Some("example.com"), 0, 10).unwrap().len(), 1);
        cleanup(&path);
    }
    ...
    fn receiver_handles_bad_json_concurrency_path_switch_and_restart() {
        ...
        let port = server.port();
        let origin = format!("chrome-extension://{BROWSER_EXTENSION_ID}");
        assert!(receiver_request(port, "POST", "/events", Some(&origin), "{").starts_with("HTTP/1.1 400"));
        ...
        assert!(receiver_request(port, "POST", "/events", Some(&origin), &switched).starts_with("HTTP/1.1 200"));
        server.shutdown().unwrap();
        assert_eq!(BrowserActivityStorage::open(&first).unwrap().recent_requests(None, 0, 20).unwrap().len(), 8);
        assert_eq!(BrowserActivityStorage::open(&second).unwrap().recent_requests(None, 0, 20).unwrap().len(), 1);
...
text = text[:insert_at] + receiver_tests + text[insert_at:]
...
constructor_start = text.index("        let (server, server_error) = match BrowserActivityServer::spawn")
constructor_end = text.index("        };", constructor_start) + len("        };")
constructor = '''        let (server, server_error) = match BrowserActivityServer::spawn(database_path.clone()) {
...
text = replace_once(
    ...
    "browser UI status chip",
)
error_marker = '''            if let Some(error) = status.last_error {
...
conflict_marker = "#[derive(Debug, Clone)]\nenum CaptureState {"
conflict = r'''struct ExistingInstanceApp {
    status: BrowserServerStatus,
}
...
impl eframe::App for ExistingInstanceApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.add_space(10.0);
            ui.label("请切换到已有窗口。为避免两个实例同时写库，本窗口不会启动第二套抓包或 Receiver。");
        });
    }
}
...
#[derive(Debug, Clone)]
enum CaptureState {'''
text = replace_once(text, conflict_marker, conflict, "GUI conflict app")
text = replace_once(
    text,
...
text = replace_once(
    ...
    "GUI startup notice field",
)
text = replace_once(
    text,
    "        app.refresh_data(false);\n        app",
    "        app.refresh_data(false);\n        if startup_notice.is_some() { app.notice = startup_notice; }\n        app",
    "GUI startup notice retention",
)
text = replace_once(
    text,
...
text = replace_once(
    ...
    + '''
            ui.label(
                egui::RichText::new(format!("实际使用：{}", PathBuf::from(self.database_path.trim()).display()))
                    .small()
                    .color(palette.muted),
            );''',
    "GUI database path label",
)
# Drop cannot call the mutable error-reporting helper safely during teardown.
text = replace_once(
    text,
...
# Isolated receiver may use port 0 and prints the actual selected port.
